...
export async function getWebsiteKeywords(
  ...
  try {
    ...
    logDebug(
      `API Info: BASE=${API_BASE}, USER=${API_USER ? "SET" : "NOT SET"}, PASS=${
        API_PASSWORD ? "SET" : "NOT SET"
      }`
    );
    ...
    logDebug(
      `Getting keywords for: ${targetSite}, forceRefresh: ${forceRefresh}`
    );
    const {
      limit = 50,
      offset = 0,
    ...
    logDebug(
      `Fetch result: total=${existingKeywords.total}, statusCode=${existingKeywords.statusCode}`
    );
    ...
    if (
      existingKeywords.statusCode === 404 ||
      existingKeywords.total === 0 ||
      forceRefresh
    ) {
      logDebug(`Upserting keywords for ${targetSite}`);
      const upsertResult = await upsertKeywords(
        targetSite,
        language_code,
        location_code,
        limit
      ...
      const freshKeywords = await fetchExistingKeywords(
        targetSite,
        limit,
        offset
      );
    ...
    logDebug(
      `Unexpected flow: statusCode=${existingKeywords.statusCode}, total=${existingKeywords.total}`
    );
    return {
      error: `Failed to get keywords (status ${existingKeywords.statusCode})`,
      keywords: [],
    };
  } catch (error) {
...
async function fetchExistingKeywords(
  ...
  limit: number,
  offset: number
): Promise<{ total: number; results: ApiKeyword[]; statusCode?: number }> {
  ...
  try {
    ...
    logDebug(
      `GET keywords response status: ${response.status} ${response.statusText}`
    );
...
async function upsertKeywords(
  ...
  try {
    ...
    logDebug(
      `POST keywords response status: ${response.status} ${response.statusText}`
    );
    ...
    if (!response.ok) {
      ...
      try {
        ...
        return {
          error: `HTTP error ${response.status}: ${responseText.substring(
            0,
            100
          )}`,
        };
...
function mapApiKeywordsToUIFormat(apiKeywords: ApiKeyword[]): KeywordResult[] {
  return apiKeywords.map((kw) => {
    ...
    let intent: "informational" | "transactional" | "navigational" =
      "informational";
    ...
    if (
      ...
      keyword.includes("shop") ||
      keyword.includes("deal")
    ) {
